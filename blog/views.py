...
def post_share(request, post_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_mail(
                f"{cd['name']} рекомендует прочитать {post.title}",
                f"Прочитать {post.title} можно по ссылке {post_url}\n\n"
    ...
    if 'query' in request.GET:
        ...
        if form.is_valid():
            ...
            results = Post.published.annotate(
                ...
                Q(search=search_query) | Q(title_similarity__gt=0.1) | Q(body_similarity__gt=0.1)
            ).order_by('-rank', '-title_similarity', '-body_similarity')
